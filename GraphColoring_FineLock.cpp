...
void color_external(long long vertex){
      long long i;
      list* head=graph_list[vertex];
      while(head!=NULL){
         sem_wait(&lock[head->data]);//keep lock on every neighbour of vertex v and for vertex v 
	 head=head->next;
      }
      color_vertex(vertex); //do coloring
      head=graph_list[vertex];
      while(head!=NULL){
         sem_post(&lock[head->data]);//unlock every neighbour of vertex v and vertex v
	 head=head->next;
      }
}
void* test(void* tem){
...
int main(){
   ...
   lock=new sem_t[row];//an array of semaphore ,for each vertex we will have seperate lock
   for(i=0;i<row;i++)
       sem_init(&lock[i],0,1); //intiliaze all semaphore to 1
   long long m;
